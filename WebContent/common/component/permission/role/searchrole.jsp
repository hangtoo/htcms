...
<div id="content">
<!-- 放内容 -->
<form action="<%=request.getContextPath()%>/permission/role_search.action" method="POST" name="theForm" onSubmit="if(this.submitted){ return false;}this.submitted=true;return true;">
		<ec:table styleClass="tableClass" items="page" tableId="ec"
				action="${pageContext.request.contextPath}/permission/role_search.action"
				...
				<ec:row highlightRow="true">
					...
					<a href="<%=request.getContextPath()%>/permission/role_edit.action?bean.id=${user.P_ID}">修改</a>&nbsp;&nbsp;
					<a onclick="return confirm('确定删除该记录吗?');" href="<%=request.getContextPath()%>/permission/roledb_delete.action?bean.id=${user.P_ID}">删除</a>&nbsp;&nbsp;
					</ec:column>
					<ec:column property="P_NAME" title="角色" />
