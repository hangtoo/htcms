...
<s:form action="admindb_save" >
	<s:hidden key="bean.id"/>
	<s:textfield key="bean.name" required="true" requiredposition="right"/>
    <s:textfield key="bean.username" required="true"/>
    <s:password key="bean.password" required="true"/>
    <s:password key="bean.repassword" required="true"/>

	<s:select list="roles" listKey="id" listValue="name" key="bean.role.id" required="true">
	</s:select>

    <s:textfield key="bean.checkcode" required="true" size="5"/>
    <s:submit key="submit" theme="ajax"/>
</s:form>
