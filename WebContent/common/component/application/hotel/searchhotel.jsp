...
<!-- 放内容 -->
<form action="<%=request.getContextPath()%>/hotel/hotel_search.action" method="POST" name="theForm" onSubmit="if(this.submitted){ return false;}this.submitted=true;return true;">
		<ec:table styleClass="tableClass" items="page" tableId="ec"
				action="${pageContext.request.contextPath}/hotel/hotel_search.action"
				...
				<ec:row highlightRow="true">
					...
					<a href="<%=request.getContextPath()%>/hotel/hotel_edit.action?bean.id=${user.P_ID}">修改</a>&nbsp;&nbsp;
					<a onclick="return confirm('确定删除该记录吗?');" href="<%=request.getContextPath()%>/hotel/hoteldb_delete.action?bean.id=${user.P_ID}">删除</a>&nbsp;&nbsp;
					</ec:column>
					<ec:column property="P_AREA" title="城市" />
					<ec:column property="P_NAME" title="酒店名称" />
