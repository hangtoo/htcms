...
<!-- 放内容 -->
<form action="<%=request.getContextPath()%>/user/user_search.action" method="POST" name="theForm" onSubmit="if(this.submitted){ return false;}this.submitted=true;return true;">
		<ec:table styleClass="tableClass" items="page" tableId="ec"
				action="${pageContext.request.contextPath}/user/user_search.action"
				...
				<ec:row highlightRow="true">
					...
					<a href="<%=request.getContextPath()%>/user/user_edit.action?bean.id=${user.P_ID}">修改</a>&nbsp;&nbsp;
					<a onclick="return confirm('确定删除该记录吗?');" href="<%=request.getContextPath()%>/user/userdb_delete.action?bean.id=${user.P_ID}">删除</a>&nbsp;&nbsp;
					</ec:column>
					<ec:column property="P_USERNAME" title="用户名" />
					<ec:column property="P_NAME" title="昵称" />
					<ec:column property="P_EMAIL" title="邮箱" />
