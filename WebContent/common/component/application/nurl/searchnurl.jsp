...
<!-- 放内容 -->
<form action="<%=request.getContextPath()%>/nurl/nurl_search.action" method="POST" name="theForm" onSubmit="if(this.submitted){ return false;}this.submitted=true;return true;">
		<ec:table styleClass="tableClass" items="page" tableId="ec"
				action="${pageContext.request.contextPath}/nurl/nurl_search.action"
				...
				<ec:row highlightRow="true">
					...
					<a href="<%=request.getContextPath()%>/nurl/nurl_edit.action?bean.id=${user.P_ID}">修改</a>&nbsp;&nbsp;
					<a onclick="return confirm('确定删除该记录吗?');" href="<%=request.getContextPath()%>/nurl/nurldb_delete.action?bean.id=${user.P_ID}">删除</a>&nbsp;&nbsp;
					</ec:column>
					<ec:column property="P_NURL" title="地址" />
					<ec:column property="P_DESC" title="描述" />
					<ec:column property="P_TAGS" title="标签" />
