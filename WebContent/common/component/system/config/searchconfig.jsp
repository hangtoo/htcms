...
<!-- 放内容 -->
<form action="<%=request.getContextPath()%>/system/config_search.action" method="POST" name="theForm" onSubmit="if(this.submitted){ return false;}this.submitted=true;return true;">
		<ec:table styleClass="tableClass" items="page" tableId="ec"
				action="${pageContext.request.contextPath}/system/config_search.action"
				...
				<ec:row highlightRow="true">
					...
					<a href="<%=request.getContextPath()%>/system/config_edit.action?bean.id=${user.P_ID}">修改</a>&nbsp;&nbsp;
					<a onclick="return confirm('确定删除该记录吗?');" href="<%=request.getContextPath()%>/system/configdb_delete.action?bean.id=${user.P_ID}">删除</a>&nbsp;&nbsp;
					</ec:column>
					<ec:column property="P_CONFIGKEY" title="参数" />
					<ec:column property="P_CONFIGVALUE" title="参数值" />
